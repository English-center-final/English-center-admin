...
function UserAction(props) {
  ...
  const handleOnUpdateStatus = async () => {
    confirm({
      icon: <ExclamationCircleOutlined />,
      content: `Bạn có muốn ${
        user?.actived ? 'huỷ kích hoạt' : 'kích hoạt'
      } tài khoản nguời dùng này không?`,
      async onOk() {
        try {
          ...
          message.success(
            `${user?.actived ? 'Huỷ kích hoạt' : 'Kích hoạt'} thành công`
          );
          dispatch(fetchUsers(query));
        } catch (error) {
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Divider />
      <Menu.Item onClick={handleOnUpdateStatus}>
        <div className="menu-adjust--center">
          <InfoCircleTwoTone />
          <span className="menu-title">
            {user?.actived ? 'Deactivated' : 'Actived'}
          </span>
        </div>
      </Menu.Item>
