...
function UserRoleModal(props) {
  ...
  const defaultValue = user.roles.filter(
    (val) => !constants.ROLE_OPTIONS.includes(val)
  );
  ...
  const onCheckAllChange = (e) => {
    ...
    const values = constants.ROLE_OPTIONS.map((record) => record.value);
  ...
  function onChange(checkedValues) {
    ...
    setCheckAll(checkedValues.length === constants.ROLE_OPTIONS.length);
  }
